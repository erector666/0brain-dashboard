...
export function Auth({ children }: { children: React.ReactNode }) {
  ...
  async function signInWithGoogle() {
    ...
    try {
      ...
      setAuthError(err instanceof Error ? err.message : "Connection failed. Try again.");
      setSigningIn(false);
    }
  }
    ...
    return (
      <div className="auth-screen">
        <div className="auth-card">
          <p style={{ color: "#667085", margin: 0 }}>Loading...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="auth-screen">
        <div className="auth-card">
          <h1>OB1 NeuroOps</h1>
          <p style={{ color: "#667085", marginBottom: 24 }}>
            Sign in to inspect agent memories
          </p>
          {authError ? <div className="error" style={{ marginBottom: 16 }}>{authError}</div> : null}
